...
#show: project.with(
  title: [SDANDART控制协议], authors: (
    ...
    - 20241226 v0.9 #new_content[扩展休眠控制指令，利用原先的保留位控制是进入休眠还是从休眠中唤醒，将非法指令重新解释为指令错误，包括对未知指令的回复和对操作失败的回复]
  ],
)
...
[4:7],[?? ?? ?? ??],[可定制的消息序列号],
[8:11],[#new_content[00/01] 00 00 00], [#new_content[第一个字节如果是0x00，就代表进入休眠，如果是0x01就代表从休眠中唤醒]]
)
]
